...
export default function ProjectsPage() {
  ...
  if (loading) {
    return (
      <div className="p-8">
        {}
        <Skeleton className="h-10 w-1/3 mb-8 bg-black/[0.08]" />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[...Array(6)].map((_, i) => (
            <Card key={i} className="w-full">
              <CardHeader className="pb-6">
                <Skeleton className="h-6 w-3/4 bg-black/[0.08]" />
              </CardHeader>
              <CardContent className="space-y-3">
                <Skeleton className="h-4 w-full bg-black/[0.08]" />
                <Skeleton className="h-4 w-2/3 bg-black/[0.08]" />
              </CardContent>
            </Card>
          ))}
        </div>
          ...
          >
            <CardHeader className="pb-6">
              <div className="flex justify-between items-start gap-3">
                ...
                <Badge
                  className={cn(getStatusColor(project.status), "px-6 py-2")}
                >
                  {project.status}
                </Badge>
              </div>
